...
export default function FormStatus({
  ...
  useEffect(() => {
    if (variant !== "toast") return;
    if (isError || status.sticky) return;
  ...
  if (variant === "inline") {
    return (
      <div
        ...
        className={`flex w-full items-start gap-2 rounded-md border px-3 py-2 text-sm ${toneInline} ${className}`}
      >
    ...
    return (
      <div className="pointer-events-none fixed bottom-6 right-6 z-[100]">
        ...
        >
          ...
          <div className="min-w-0 whitespace-pre-line text-base">{status.msg}</div>
          {onClose ? (
            <button
              type="button"
              onClick={onClose}
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50">
      ...
      >
        ...
        <div className="whitespace-pre-line px-4 py-4 text-gray-800">{status.msg}</div>
