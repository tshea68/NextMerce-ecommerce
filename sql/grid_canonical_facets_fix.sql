-- Rebuild the view cleanly so it has BOTH part_type and canonical_part_type.
-- CASCADE is intentional here because the facet functions depend on grid_all
-- and we recreate them immediately below.

drop view if exists public.grid_all cascade;

create view public.grid_all as
select
    'p:' || p.id::text as rid,
